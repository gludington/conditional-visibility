...
describe('DefaultConditionaVisibilitySystem', () => {
    ...
    describe('Testing if something is visible', () => {
        let flags:any = {};
        let token:any = { 
            data: { flags: { 'conditional-visibility': {}}},
            getFlag:(scope, key) => {
        ...
        it('if token has no effects, no flags are needed to see', () => {
            ...
            //@ts-ignore
            expect(system.seeObscured(token, token.getFlag(Constants.MODULE_NAME, Constants.VISIBLE_STATUS_FIELD), flags)).toBe(true);
            //@ts-ignore
            expect(system.seeInDarkness(token, token.getFlag(Constants.MODULE_NAME, Constants.VISIBLE_STATUS_FIELD), flags)).toBe(true);
            //@ts-ignore
        ...
        describe('if a token has multiple effects', () => {
            ...
            beforeEach(() => {
                flags = {};
                    token.data.flags = { 'conditional-visibility': { 'visiblestatus': {invisible:true, obscured:true}}}
            });
        ...
        describe('if token is obscured', () => {
            ...
            beforeEach(() => {
                flags = {};
                token.data.flags = { 'conditional-visibility': { 'visiblestatus': { obscured:true}}};
            });
            it ('empty capabilities cannot see it', () => {
                flags.seeinvisible = false;
                //@ts-ignore
                expect(system.seeObscured(token, token.getFlag(Constants.MODULE_NAME, Constants.VISIBLE_STATUS_FIELD), flags)).toBe(false);
            });
        ...
        describe('if token is in darkness', () => {   
            beforeEach(() => {
                flags = {};
                token.data.flags = { 'conditional-visibility': { 'visiblestatus': {indarkness:true}}};
            });
            it ('empty capabilities cannot see it', () => {
                //@ts-ignore
                expect(system.seeInDarkness(token, token.getFlag(Constants.MODULE_NAME, Constants.VISIBLE_STATUS_FIELD), flags)).toBe(false);
            });
